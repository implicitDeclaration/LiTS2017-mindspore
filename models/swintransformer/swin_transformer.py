...
from models.swintransformer.misc import *
...
class WindowPartitionConstruct(nn.Cell):
    ...
    def construct(self, x):
        ...
        # x = self.reshape(x, (-1, self.window_size, self.window_size, C))
...
class WindowReverseConstruct(nn.Cell):
    ...
    def construct(self, windows, window_size, H, W):
        ...
        x = self.reshape1(windows, (B, H // window_size, W // window_size, window_size, window_size, -1))
        x = self.transpose(x, (0, 1, 3, 2, 4, 5))
        x = self.reshape2(x, (B, H, W, -1))
        return x
...
class WindowAttention(nn.Cell):
    ...
    def construct(self, x, mask=None):
        ...
        """
        B_, N, C = x.shape
        qkv = self.reshape(self.qkv(x), (B_, N, 3, self.num_heads, C // self.num_heads))
        qkv = self.transpose(qkv, (2, 0, 3, 1, 4))
        q, k, v = qkv[0], qkv[1], qkv[2]  # make npscript happy (cannot use tensor as tuple)

        q = q * self.scale
        attn = self.matmul(q, self.transpose(k, (0, 1, 3, 2)))
        temp = self.relative_position_bias_table[reshape(self.relative_position_index, (-1,))]
        relative_position_bias = self.reshape(temp,
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn = P.Reshape()(attn, (B_ // nW, nW, self.num_heads, N, N,)) + expand_dims(expand_dims(mask, 1), 0)
        ...
        x = reshape(transpose(matmul(attn, v), (0, 2, 1, 3)), (B_, N, C))
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class SwinTransformerBlock(nn.Cell):
    ...
    def construct(self, x):
        ...
        # cyclic shift
        if self.shift_size > 0:
            shifted_x = numpy.roll(x, shift=(-self.shift_size, -self.shift_size), axis=(1, 2))
        else:
            shifted_x = x
        ...
        if self.shift_size > 0:
            x = numpy.roll(shifted_x, shift=(self.shift_size, self.shift_size), axis=(1, 2))
        else:
            x = shifted_x
...
class PatchMerging(nn.Cell):
    ...
    def construct(self, x):
        ...
        x = P.Concat(-1)([x0, x1, x2, x3])  # B H/2 W/2 4*C
        x = P.Reshape()(x, (B, -1, 4 * C,))  # B H/2*W/2 4*C
...
class PatchEmbed(nn.Cell):
    ...
    def construct(self, x):
        ...
        x = self.reshape(self.proj(x), (B, self.embed_dim, -1))  # B Ph*Pw C
...
class SwinTransformer(nn.Cell):
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x)
        x = self.norm(x)  # B L C
        x = self.avgpool(transpose(x, (0, 2, 1)), 2)  # B C 1
        x = P.Flatten()(x)
        return x
